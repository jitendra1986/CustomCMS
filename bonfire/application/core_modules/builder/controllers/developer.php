    ...
    private function validate_form($field_total = 0) {
        ...
        if ($this->input->post('module_db')) {
            ...
            if ($this->input->post('module_db') == 'new') {
                ...
                $this->form_validation->set_rules("use_status", 'Use Status Field', "trim|xss_clean|alpha");
                $this->form_validation->set_rules("status_field", 'Status Field Name', "trim|xss_clean|alpha_dash");
                $this->form_validation->set_rules("use_position", 'Use Position Field', "trim|xss_clean|alpha");
                $this->form_validation->set_rules("status_field", 'Status Field Name', "trim|xss_clean|alpha_dash");
            ...
            for ($counter = 1; $field_total >= $counter; $counter++) {
                ...
                if (!empty($label) && !($field_type == 'TEXT' OR $field_type == 'MEDIUMTEXT' OR $field_type == 'LONGTEXT' OR $field_type == 'BOOL' OR $field_type == 'DATE' OR $field_type == 'TIME' OR $field_type == 'DATETIME' OR $field_type == 'TIMESTAMP' OR $field_type == 'YEAR' OR $field_type == 'TINYBLOB' OR $field_type == 'BLOB' OR $field_type == 'MEDIUMBLOB' OR $field_type == 'LONGBLOB')) {
