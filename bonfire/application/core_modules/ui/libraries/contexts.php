...
//end get_contexts()
    ...
    public static function render_menu($mode = 'text', $order_by = 'normal', $top_level_only = FALSE) {
        ...
        $key = array_search('reports', $contexts);
        if (false !== $key) {
            unset($contexts[$key]);
        }
        ...
        foreach ($contexts as $context) {
            if (has_permission('Site.' . ucfirst($context) . '.View') == true || permission_exists('Site.' . ucfirst($context) . '.View') == false) {
                ...
                $nav .= "<li> <a href='{$url}' title='{$title}' data-toggle='collapse-next' class='has-submenu'><em class='fa fa-list-alt'></em>";
                ...
                if ($mode == 'text' || $mode == 'both') {
                    $nav .= "<span class='item-text'>{$title}</span>";
                }
                $nav .= "</a>";
...
//end render_mobile_navs()
    ...
    public function context_nav($context = NULL, $class = 'dropdown-menu', $ignore_ul = FALSE) {
        ...
        $ci = &get_instance();
        $class = "";
        if ($ci->uri->segment(2) == "$context")
            $class = "in";
        // Do we have any actions?
        if (!count(self::$actions)) {
            return '<ul class="nav collapse' . $class . '"></ul>';
        }
...
//end set_attrs()
    ...
    public static function build_sub_menu($context, $ignore_ul = FALSE) {
        ...
        if (!$ignore_ul) {
            $list = "<ul class='nav collapse " . $class . "'>\n";
        }
        ...
        foreach (self::$menu as $topic_name => $topic) {
            ...
            if (count($topic) > 1) {
                $list .= '<li class="no-link parent-menu"><a href="#" class="no-link parent-menu">  <span class="item-text">' . ucwords($topic_name) . '</span></a>';
                $list .= '<ul>';
